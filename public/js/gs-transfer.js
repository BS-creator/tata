  ...
  var TransferServerURL           = sessionStorage.getItem('TransferServerURL'),
      ...
      getFilesID                  = function () {
        ...
        _.forEach(array, function (item) {
          if (item[1].display) it = $(item[1].display); else it = $(item[1]);
    ...
    return table.rows('.active', {search: 'applied'}).data();
      ...
      templateMenu                = function (menu) {
        ...
        var htmlLeafNode = '', htmlCategoryNode = '', currentCat = '', currentCatLabel = '', createLeafNode = _.template($('#menuL3').html()), createCategoryNode = _.template($('#menuL2').html());

        _.forEach(menu, function (catArray) {
          _.forEach(catArray, function (item) {

            htmlLeafNode += createLeafNode({
              referenceDocument: parseInt(item.referenceDocument), typeDocument: labelDocI18n(item)
            });
          ...
          htmlCategoryNode += createCategoryNode({
            categoryNumber: currentCat, categoryName: currentCatLabel, leafNode: htmlLeafNode
          });
          htmlLeafNode = '';
        });
      ...
      templateHeader              = function templateHeader() {
        var tpl = _.template($('#headertpl').html());
        $(TABLEID).find('thead').html(tpl(i18n[lang].col));
      },

      templateTable               = function templateTable() {
      ...
      createDataTable             = function createDataTable() {
