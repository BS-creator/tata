...
(function pdfjsWrapper() {
  ...
  var AnnotationType = {WIDGET: 1, TEXT: 2, LINK: 3};
  ...
  Object.defineProperty(PDFJS, "isLittleEndian", {
    configurable: true, get: function PDFJS_isLittleEndian() {
  ...
  Object.defineProperty(PDFJS, "hasCanvasTypedArrays", {
    configurable: true, get: function PDFJS_hasCanvasTypedArrays() {
  ...
  (function PromiseClosure() {
    ...
    Promise.prototype = {
      ...
      then               : function Promise_then(onResolve, onReject) {
        ...
        this._handlers.push({thisPromise: this, onResolve: onResolve, onReject: onReject, nextPromise: nextPromise});
        HandlerManager.scheduleHandlers(this);
        return nextPromise
      },
      ...
      if (data.isReply) {
        ...
        if (data.callbackId) {
          Promise.resolve().then(function () {
            ...
            comObj.postMessage({isReply: true, callbackId: data.callbackId, data: result})
          }, function (reason) {
            comObj.postMessage({isReply: true, callbackId: data.callbackId, error: reason})
          })
        } else {
  ...
  MessageHandler.prototype = {
    on                : function messageHandlerOn(actionName, handler, scope) {
      ...
      var message = {action: actionName, data: data};
      this.postMessage(message, transfers)
    }, sendWithPromise: function messageHandlerSendWithPromise(actionName, data, transfers) {
      var callbackId = this.callbackIndex++;
      var message = {action: actionName, data: data, callbackId: callbackId};
  ...
  PDFJS.getDocument = function getDocument(source, pdfDataRangeTransport, passwordCallback, progressCallback) {
    ...
    transport = new WorkerTransport(workerInitializedCapability, workerReadyCapability, pdfDataRangeTransport, progressCallback);
    ...
    PDFPageProxy.prototype = {
      get pageNumber() {
        ...
        if (!intentState.opListReadCapability) {
          ...
          intentState.operatorList = {fnArray: [], argsArray: [], lastChunk: false};
          this.transport.messageHandler.send("RenderPageRequest", {pageIndex: this.pageIndex, intent: renderingIntent})
        }
        return intentState.opListReadCapability.promise
      }, getTextContent  : function PDFPageProxy_getTextContent() {
        return this.transport.messageHandler.sendWithPromise("GetTextContent", {pageIndex: this.pageNumber - 1})
      }, destroy         : function PDFPageProxy_destroy() {
        this.pendingDestroy = true;
        this._tryDestroy()
      }, _tryDestroy     : function PDFPageProxy__destroy() {
        if (!this.pendingDestroy || Object.keys(this.intentStates).some(function (intent) {
  ...
  var WorkerTransport = function WorkerTransportClosure() {
    function WorkerTransport(workerInitializedCapability, workerReadyCapability, pdfDataRangeTransport, progressCallback) {
    ...
    WorkerTransport.prototype = {
      destroy               : function WorkerTransport_destroy() {
        ...
        messageHandler.on("NeedPassword", function transportPassword(data) {
          ...
          this.workerReadyCapability.reject(data.exception.message, data.exception)
        }, this);
        messageHandler.on("IncorrectPassword", function transportBadPass(data) {
          if (this.passwordCallback) {
            return this.passwordCallback(updatePassword, PasswordResponses.INCORRECT_PASSWORD)
          }
          this.workerReadyCapability.reject(data.exception.message, data.exception)
        }, this);
        ...
        messageHandler.on("commonobj", function transportObj(data) {
          ...
          switch (type) {
            case"Font":
              ...
              FontLoader.bind([font], function fontReady(fontObjs) {
        ...
        messageHandler.on("DocProgress", function transportDocProgress(data) {
          if (this.progressCallback) {
            this.progressCallback({loaded: data.loaded, total: data.total})
          }
        }, this);
        ...
        return this.messageHandler.sendWithPromise("GetAnnotations", {pageIndex: pageIndex})
        ...
        return this.messageHandler.sendWithPromise("GetMetadata", null).then(function transportMetadata(results) {
          return {info: results[0], metadata: results[1] ? new PDFJS.Metadata(results[1]) : null}
        })
  ...
  var PDFObjects = function PDFObjectsClosure() {
    ...
    PDFObjects.prototype = {
      ensureObj    : function PDFObjects_ensureObj(objId) {
        ...
        var obj = {capability: createPromiseCapability(), data: null, resolved: false};
        this.objs[objId] = obj;
        return obj
    ...
    function fixMetadata(meta) {
      return meta.replace(/>\\376\\377([^<]+)/g, function (all, codes) {
        ...
        for (var i = 0; i < bytes.length; i += 2) {
          ...
          chars += code >= 32 && code < 127 && code !== 60 && code !== 62 && code !== 38 && false ? String.fromCharCode(code) : "&#x" + (65536 + code).toString(16).substring(1) + ";"
        }
        return ">" + chars
      })
    }
  ...
  var CanvasGraphics = function CanvasGraphicsClosure() {
    ...
    function putBinaryImageData(ctx, imgData) {
      ...
      if (imgData.kind === ImageKind.GRAYSCALE_1BPP) {
        ...
        var black = PDFJS.isLittleEndian || !PDFJS.hasCanvasTypedArrays ? 4278190080 : 255;
        for (i = 0; i < totalChunks; i++) {
          thisChunkHeight = i < fullChunks ? fullChunkHeight : partialChunkHeight;
          destPos = 0;
    ...
    function putBinaryImageMask(ctx, imgData) {
      ...
      for (var i = 0; i < totalChunks; i++) {
        var thisChunkHeight = i < fullChunks ? fullChunkHeight : partialChunkHeight;
        var destPos = 3;
    ...
    function composeSMaskLuminosity(maskData, layerData) {
      var length = maskData.length;
      for (var i = 3; i < length; i += 4) {
        var y = maskData[i - 3] * 77 + maskData[i - 2] * 152 + maskData[i - 1] * 28 | 0;
    ...
    function composeSMask(ctx, smask, layerCtx) {
      ...
      if (WebGLUtils.isEnabled) {
        var composed = WebGLUtils.composeSMask(layerCtx.canvas, mask, {subtype: smask.subtype, backdrop: backdrop});
    ...
    CanvasGraphics.prototype = {
      ...
      beginSMaskGroup             : function CanvasGraphics_beginSMaskGroup() {
        ...
        var scratchCanvas = CachedCanvases.getCanvas(cacheId, drawnWidth, drawnHeight, true);
      ...
      paintChar                   : function CanvasGraphics_paintChar(character, x, y) {
        ...
        if (isAddToPathSet) {
          ...
          paths.push({transform: ctx.mozCurrentTransform, x: x, y: y, fontSize: fontSize, addToPath: addToPath})
        }
      },
      ...
      showType3Text               : function CanvasGraphics_showType3Text(glyphs) {
        ...
        for (i = 0; i < glyphsLength; ++i) {
          ...
          if (glyph === null) {
            ...
            var spacingLength = -glyph * .001 * fontSize;
      ...
      getColorN_Pattern           : function CanvasGraphics_getColorN_Pattern(IR) {
        ...
        if (IR[0] === "TilingPattern") {
          var color = IR[1];
          pattern = new TilingPattern(IR, color, this.ctx, this.objs, this.commonObjs, this.baseTransform)
        } else {
      ...
      paintImageMaskXObject       : function CanvasGraphics_paintImageMaskXObject(img) {
        ...
        if (COMPILE_TYPE3_GLYPHS && glyph && !("compiled"in glyph)) {
          ...
          if (width <= MAX_SIZE_TO_COMPILE && height <= MAX_SIZE_TO_COMPILE) {
            glyph.compiled = compileType3Glyph({data: img.data, width: width, height: height})
          } else {
            glyph.compiled = null
          }
        }
        ...
        var fillColor = this.current.fillColor;
        maskCtx.fillStyle = fillColor && fillColor.hasOwnProperty("type") && fillColor.type === "Pattern" ? fillColor.getPattern(maskCtx, this) : fillColor;
      ...
      paintInlineImageXObjectGroup: function CanvasGraphics_paintInlineImageXObjectGroup(imgData, map) {
        ...
        for (var i = 0, ii = map.length; i < ii; i++) {
          ...
          if (this.imageLayer) {
            ...
            this.imageLayer.appendImage({imgData: imgData, left: position[0], top: position[1], width: w, height: h})
          }
          ctx.restore()
        }
      },
  ...
  var createMeshCanvas = function createMeshCanvasClosure() {
    ...
    function createMeshCanvas(bounds, combinesScale, coords, colors, figures, backgroundColor) {
    ...
    fromIR: function Mesh_fromIR(raw) {
      ...
      return {
        type: "Pattern", getPattern: function Mesh_getPattern(ctx, owner, shadingFill) {
          ...
          var temporaryPatternCanvas = createMeshCanvas(bounds, scale, coords, colors, figures, shadingFill ? null : background);
          if (!shadingFill) {
  ...
  ShadingIRs.Dummy = {
    fromIR: function Dummy_fromIR() {
      return {
        type: "Pattern", getPattern: function Dummy_fromIR_getPattern() {
          return "hotpink"
        }
      }
    }
  ...
  var FontLoader = {
    ...
    loadTestFontId            : 0,
    loadingContext            : {requests: [], nextRequestId: 0},
    ...
    queueLoadingCallback      : function FontLoader_queueLoadingCallback(callback) {
      ...
      var request = {id: requestId, complete: LoadLoader_completeRequest, callback: callback, started: Date.now()};
