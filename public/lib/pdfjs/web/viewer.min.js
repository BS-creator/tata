...
var SidebarView = {NONE: 0, THUMBS: 1, OUTLINE: 2, ATTACHMENTS: 3};
var Preferences = {
  prefs                       : Object.create(DEFAULT_PREFERENCES),
  isInitializedPromiseResolved: false,
  initializedPromise          : null,
  initialize                  : function preferencesInitialize() {
    return this.initializedPromise = this._readFromStorage(DEFAULT_PREFERENCES).then(function (prefObj) {
  ...
  function next() {
    ...
    if (++index < canvases.length) {
      ...
      if (typeof canvas.mozPrintCallback === "function") {
        canvas.mozPrintCallback({context: canvas.getContext("2d"), abort: abort, done: next})
      } else {
        next()
      }
    } else {
  ...
  PDFFindController.prototype = {
    setFindBar          : function PDFFindController_setFindBar(findBar) {
      ...
      function extractPageText(pageIndex) {
        self.pdfPageSource.pages[pageIndex].getTextContent().then(function textContentResolved(textContent) {
      ...
      if (this.integratedFind) {
        FirefoxCom.request("updateFindControlState", {result: state, findPrevious: previous});
        return
      }
...
var PDFHistory = {
  initialized               : false, initialDestination: null, initialize: function pdfHistoryInitialize(fingerprint) {
    ...
    if (params.dest && !params.hash) {
      params.hash = this.current.hash && this.current.dest && this.current.dest === params.dest ? this.current.hash : PDFView.getDestinationHash(params.dest).split("#")[1]
    }
    if (params.page) {
      params.page |= 0
    }
...
var SecondaryToolbar = {
  ...
  initialize             : function secondaryToolbarInitialize(options) {
    ...
    var elements = [{element: this.toggleButton, handler: this.toggle}, {
...
var DELAY_BEFORE_RESETTING_SWITCH_IN_PROGRESS = 1e3;
var PresentationMode = {
  ...
  request               : function presentationModeRequest() {
    ...
    this.args = {page: PDFView.page, previousScale: PDFView.currentScaleValue};
    return true
  },
...
var GrabToPan = function GrabToPanClosure() {
  ...
  GrabToPan.prototype = {
    CSS_CLASS_GRAB : "grab-to-pan-grab", activate: function GrabToPan_activate() {
      if (!this.active) {
        this.active = true;
...
var HandTool = {
  initialize              : function handToolInitialize(options) {
    ...
    this.handTool = new GrabToPan({
      element: options.container, onActiveChanged: function (isActive) {
        if (!toggleHandTool) {
          return
        }
        if (isActive) {
          toggleHandTool.title = mozL10n.get("hand_tool_disable.title", null, "Disable hand tool");
          toggleHandTool.firstElementChild.textContent = mozL10n.get("hand_tool_disable_label", null, "Disable hand tool")
        } else {
          toggleHandTool.title = mozL10n.get("hand_tool_enable.title", null, "Enable hand tool");
          toggleHandTool.firstElementChild.textContent = mozL10n.get("hand_tool_enable_label", null, "Enable hand tool")
        }
      }
    });
    if (toggleHandTool) {
...
var DocumentProperties = {
  ...
  getProperties        : function documentPropertiesGetProperties() {
    ...
    PDFView.pdfDocument.getMetadata().then(function (data) {
      ...
      }, {field: this.modificationDateField, content: this.parseDate(data.info.ModDate)}, {
  ...
  parseDate            : function documentPropertiesParseDate(inputDate) {
    ...
    return mozL10n.get("document_properties_date_string", {date: dateString, time: timeString}, "{{date}}, {{time}}")
  }
};
var PDFView = {
  pages                : [],
  ...
  initialize           : function pdfViewInitialize() {
    ...
    this.findController = new PDFFindController({pdfPageSource: this, integratedFind: this.supportsIntegratedFind});
  ...
  _setScaleUpdatePages : function pdfView_setScaleUpdatePages(newScale, newValue, resetAutoSettings, noScroll) {
    ...
    if (!noScroll) {
      ...
      if (this.currentPosition && !IGNORE_CURRENT_POSITION_ON_ZOOM) {
        ...
        dest = [null, {name: "XYZ"}, this.currentPosition.left, this.currentPosition.top, null]
      }
  ...
  get loadingBar() {
    ...
    Object.defineProperty(this, "loadingBar", {value: bar, enumerable: true, configurable: true, writable: false});
    return bar
  },
  get isHorizontalScrollbarEnabled() {
    return PresentationMode.active ? false : this.container.scrollWidth > this.container.clientWidth
  },
  ...
  open                 : function pdfViewOpen(url, scale, password, pdfDataRangeTransport, args) {
    ...
    PDFJS.getDocument(parameters, pdfDataRangeTransport, passwordNeeded, getDocumentProgress).then(function getDocumentCallback(pdfDocument) {
  ...
  load                 : function pdfViewLoad(pdfDocument, scale) {
    ...
    var showPreviousViewOnLoadPromise = Preferences.get("showPreviousViewOnLoad").then(function (prefValue) {
      showPreviousViewOnLoad = prefValue
    });
    var defaultZoomValue;
    var defaultZoomValuePromise = Preferences.get("defaultZoomValue").then(function (prefValue) {
      defaultZoomValue = prefValue
    });
    var storePromise = store.initializedPromise;
    Promise.all([firstPagePromise, storePromise, showPreviousViewOnLoadPromise, defaultZoomValuePromise]).then(function resolved() {
      var storedHash = null;
    ...
    Promise.all(promises).then(function () {
      pdfDocument.getOutline().then(function (outline) {
        ...
        if (outline && self.preferenceSidebarViewOnLoad === SidebarView.OUTLINE) {
      ...
      pdfDocument.getAttachments().then(function (attachments) {
        ...
        if (attachments && self.preferenceSidebarViewOnLoad === SidebarView.ATTACHMENTS) {
    ...
    pdfDocument.getMetadata().then(function (data) {
      ...
      console.log("PDF " + pdfDocument.fingerprint + " [" + info.PDFFormatVersion + " " + (info.Producer || "-").trim() + " / " + (info.Creator || "-").trim() + "]" + " (PDF.js: " + (PDFJS.version || "-") + (!PDFJS.disableWebGL ? " [WebGL]" : "") + ")");
      var pdfTitle;
  ...
  cleanup              : function pdfViewCleanup() {
    for (var i = 0, ii = this.pages.length; i < ii; i++) {
      if (this.pages[i] && this.pages[i].renderingState !== RenderingStates.FINISHED) {
        this.pages[i].reset()
      }
    }
  ...
  setHash              : function pdfViewSetHash(hash) {
    var validFitZoomValues = ["Fit", "FitB", "FitH", "FitBH", "FitV", "FitBV", "FitR"];
    if (!hash) {
      return
    }
    if (hash.indexOf("=") >= 0) {
      ...
      if ("zoom"in params) {
        ...
        dest = [null, {name: destName}, zoomArgs.length > 1 ? zoomArgs[1] | 0 : null, zoomArgs.length > 2 ? zoomArgs[2] | 0 : null, zoomArg]
      }
      if (dest) {
      ...
      if ("pagemode"in params) {
        if (params.pagemode === "thumbs" || params.pagemode === "bookmarks" || params.pagemode === "attachments") {
  ...
  getVisibleElements   : function pdfViewGetVisibleElements(scrollEl, views, sortByVisibility) {
    ...
    for (var i = 0, ii = views.length; i < ii; ++i) {
      ...
      visible.push({id: view.id, x: currentWidth, y: currentHeight, view: view, percent: percentHeight})
    }
    var first = visible[0];
  ...
  mouseScroll          : function pdfViewMouseScroll(mouseScrollDelta) {
    ...
    if (Math.abs(this.mouseScrollDelta) >= PAGE_FLIP_THRESHOLD) {
      var PageFlipDirection = {UP: -1, DOWN: 1};
...
var PageView = function pageView(container, id, scale, navigateTo, defaultViewport) {
  ...
  this.reset = function pageViewReset(keepAnnotations) {
    ...
    for (var i = div.childNodes.length - 1; i >= 0; i--) {
      ...
      if (this.zoomLayer && this.zoomLayer === node || keepAnnotations && this.annotationLayer === node) {
        continue
      }
  ...
  this.update = function pageViewUpdate(scale, rotation) {
    ...
    var totalRotation = (this.rotation + this.pdfPageRotate) % 360;
    this.viewport = this.viewport.clone({scale: this.scale * CSS_UNITS, rotation: totalRotation});
    ...
    if (this.canvas && (PDFJS.useOnlyCssZoom || this.hasRestrictedScaling && isScalingRestricted)) {
    ...
    pdfPage.getAnnotations().then(function (annotationsData) {
      ...
      if (self.annotationLayer) {
        for (i = 0, ii = annotationsData.length; i < ii; i++) {
          ...
          element = self.annotationLayer.querySelector('[data-annotation-id="' + data.id + '"]');
          if (element) {
  ...
  this.scrollIntoView = function pageViewScrollIntoView(dest) {
    ...
    var pageWidth = (changeOrientation ? this.height : this.width) / this.scale / CSS_UNITS;
    var pageHeight = (changeOrientation ? this.width : this.height) / this.scale / CSS_UNITS;
    var scale = 0;
    switch (dest[1].name) {
      case"XYZ":
        x = dest[2];
  ...
  this.draw = function pageviewDraw(callback) {
    ...
    function pageViewDrawCallback(error) {
      ...
      if (error) {
        PDFView.error(mozL10n.get("rendering_error", null, "An error occurred while rendering the page."), error)
      }
      ...
      var event = document.createEvent("CustomEvent");
      event.initCustomEvent("pagerender", true, true, {pageNumber: pdfPage.pageNumber});
    ...
    this.renderTask.promise.then(function pdfPageRenderCallback() {
      ...
      if (textLayer) {
        self.getTextContent().then(function textContentResolved(textContent) {
  ...
  this.beforePrint = function pageViewBeforePrint() {
    ...
    canvas.mozPrintCallback = function (obj) {
      ...
      var renderContext = {canvasContext: ctx, viewport: viewport, intent: "print"};
...
var ThumbnailView = function thumbnailView(container, id, defaultViewport) {
  ...
  this.update = function thumbnailViewUpdate(rotation) {
    ...
    var totalRotation = (this.rotation + this.pdfPageRotate) % 360;
    this.viewport = this.viewport.clone({scale: 1, rotation: totalRotation});
    this.pageWidth = this.viewport.width;
    this.pageHeight = this.viewport.height;
    this.pageRatio = this.pageWidth / this.pageHeight;
    this.canvasHeight = this.canvasWidth / this.pageWidth * this.pageHeight;
  ...
  this.getPageDrawContext = function thumbnailViewGetPageDrawContext() {
    ...
    canvas.setAttribute("aria-label", mozL10n.get("thumb_page_canvas", {page: id}, "Thumbnail of Page {{page}}"));
    div.setAttribute("data-loaded", true);
  ...
  this.draw = function thumbnailViewDraw(callback) {
    ...
    var renderContext = {
      canvasContext: ctx, viewport: drawViewport, continueCallback: function (cont) {
    ...
    this.pdfPage.render(renderContext).promise.then(function pdfPageRenderCallback() {
      self.renderingState = RenderingStates.FINISHED;
      callback()
    }, function pdfPageRenderError(error) {
      self.renderingState = RenderingStates.FINISHED;
  ...
  this.setImage = function thumbnailViewSetImage(img) {
    ...
    var MAX_SCALE_FACTOR = 2;
    if (Math.max(img.width / ctx.canvas.width, img.height / ctx.canvas.height) > MAX_SCALE_FACTOR) {
      reducedWidth >>= 1;
      reducedHeight >>= 1;
      reducedImage = getTempCanvas(reducedWidth, reducedHeight);
      var reducedImageCtx = reducedImage.getContext("2d");
      reducedImageCtx.drawImage(img, 0, 0, img.width, img.height, 0, 0, reducedWidth, reducedHeight);
      while (Math.max(reducedWidth / ctx.canvas.width, reducedHeight / ctx.canvas.height) > MAX_SCALE_FACTOR) {
        reducedImageCtx.drawImage(reducedImage, 0, 0, reducedWidth, reducedHeight, 0, 0, reducedWidth >> 1, reducedHeight >> 1);
        reducedWidth >>= 1;
        reducedHeight >>= 1
      }
    }
    ctx.drawImage(reducedImage, 0, 0, reducedWidth, reducedHeight, 0, 0, ctx.canvas.width, ctx.canvas.height);
    this.hasImage = true
  }
};
ThumbnailView.tempImageCache = null;
...
var TextLayerBuilder = function TextLayerBuilderClosure() {
  ...
  TextLayerBuilder.prototype = {
    renderLayer              : function TextLayerBuilder_renderLayer() {
      ...
      var textDivs = this.textDivs;
      var prevEnd = null;
      var isSelectedPage = this.findController === null ? false : this.pageIdx === this.findController.selected.pageIdx;
      ...
      for (var i = i0; i < i1; i++) {
        ...
        if (isSelected && !this.isViewerInPresentationMode) {
          scrollIntoView(textDivs[begin.divIdx], {top: FIND_SCROLL_OFFSET_TOP, left: FIND_SCROLL_OFFSET_LEFT})
        }
...
function updateViewarea() {
  ...
  var topLeft = currentPage.getPagePoint(container.scrollLeft - firstPage.x, container.scrollTop - firstPage.y);
...
window.addEventListener("resize", function webViewerResize(evt) {
  if (PDFView.initialized && (document.getElementById("pageWidthOption").selected || document.getElementById("pageFitOption").selected || document.getElementById("pageAutoOption").selected)) {
...
window.addEventListener("scalechange", function scalechange(evt) {
  ...
  if (!predefinedValueFound) {
    customScaleOption.textContent = Math.round(evt.scale * 1e4) / 100 + "%";
...
window.addEventListener("keydown", function keydown(evt) {
  ...
  if (cmd === 0) {
    switch (evt.keyCode) {
      ...
      case 33:
      case 8:
        if (!PresentationMode.active && PDFView.currentScaleValue !== "page-fit") {
          break
        }
      case 37:
      ...
      case 36:
        ...
        break;
      case 35:
        if (PresentationMode.active || PDFView.pdfDocument && PDFView.page < PDFView.pdfDocument.numPages) {
          PDFView.page = PDFView.pdfDocument.numPages;
          handled = true
        }
        break;
