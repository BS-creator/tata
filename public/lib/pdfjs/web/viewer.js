...
(function mozPrintCallbackPolyfillClosure() {
  ...
  window.addEventListener('keydown', function (event) {
    ...
    if (event.keyCode === 80/*P*/ && (event.ctrlKey || event.metaKey) && !event.altKey && (!event.shiftKey || window.chrome || window.opera)) {
      window.print();
      if (hasAttachEvent) {
...
var PresentationMode = {
  ...
  request: function presentationModeRequest() {
    if (!PDFView.supportsFullscreen || this.isFullscreen || !this.viewer.hasChildNodes()) {
      return false;
    }
...
var OverlayManager = {
  ...
  register: function overlayManagerRegister(name,
    ...
    return new Promise(function (resolve) {
      ...
      if (!name || !(element = document.getElementById(name)) || !(container = element.parentNode)) {
...
var PDFView = {
  ...
  _setScaleUpdatePages: function pdfView_setScaleUpdatePages(newScale, newValue, resetAutoSettings, noScroll) {
...
var TextLayerBuilder = (function TextLayerBuilderClosure() {
  ...
  TextLayerBuilder.prototype = {
    ...
    setupRenderLayoutTimer: function TextLayerBuilder_setupRenderLayoutTimer() {
...
window.addEventListener('keydown', function keydown(evt) {
  ...
  if (!handled && !PresentationMode.active) {
    ...
    if (evt.keyCode >= 33 && evt.keyCode <= 40 && !PDFView.container.contains(curElement)) {
