(function (window, document) {
  ...
  window.sweetAlert = window.swal = function () {
    ...
    var onButtonEvent = function (e) {
      var target = e.target || e.srcElement, targetedConfirm = target.className === "confirm", modalIsVisible = hasClass(modal, "visible"), doneFunctionExists = params.doneFunction && modal.getAttribute("data-has-done-function") === "true";
      switch (e.type) {
        case"mouseover":
          if (targetedConfirm) {
            e.target.style.backgroundColor = colorLuminance(params.confirmButtonColor, -.04)
          }
          break;
        case"mouseout":
          if (targetedConfirm) {
        ...
        case"mousedown":
          if (targetedConfirm) {
            e.target.style.backgroundColor = colorLuminance(params.confirmButtonColor, -.14)
          }
          break;
        case"mouseup":
          if (targetedConfirm) {
    ...
    document.onclick = function (e) {
      ...
      var clickedOnModal = modal === target, clickedOnModalChild = isDescendant(modal, e.target), modalIsVisible = hasClass(modal, "visible"), outsideClickIsAllowed = modal.getAttribute("data-allow-ouside-click") === "true";
      if (!clickedOnModal && !clickedOnModalChild && modalIsVisible && outsideClickIsAllowed) {
        closeModal()
      }
    };
    var $okButton = modal.querySelector("button.confirm"), $cancelButton = modal.querySelector("button.cancel"), $modalButtons = modal.querySelectorAll("button:not([type=hidden])");
